...
export default function ExplorePage() {
  ...
  const categories = ["All", "Trading", "Analysis", "DeFi", "NFT", "Automation", "Gaming", "Social", "Research"];
        ...
        <div className="flex items-center justify-between mb-6 text-[10px] uppercase text-slate-500 tracking-widest font-mono">
          <div>
            {loading ? (
              ...
              <>Matches Found: <span className="text-white">{filteredAgents.length}</span></>
            )}
          </div>
          <div className="flex items-center gap-2">
            <div className={`w-2 h-2 rounded-full ${loading ? "bg-yellow-500 animate-pulse" : filteredAgents.length > 0 ? "bg-[#00ff9d] animate-pulse" : "bg-red-500"}`} />
            Status: {loading ? "LOADING" : filteredAgents.length > 0 ? "ONLINE" : "NO DATA"}
          </div>
        </div>
        ...
        {error ? (
          ...
          >
            {loading ? (
              // Skeleton loaders
              [...Array(8)].map((_, i) => (
                <div key={i} className="bg-[#0a0a0a] border border-[#333] flex flex-col h-full overflow-hidden animate-pulse">
                  {/* Header skeleton */}
