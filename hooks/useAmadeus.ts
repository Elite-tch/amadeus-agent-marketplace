...
export type ProviderStatus = 'checking' | 'not-found' | 'available';
...
export function useAmadeus(): UseAmadeusResult {
  ...
  const tryDetectProvider = useCallback(async () => {
    ...
    if (window.amadeus && window.amadeus.isAmadeus) {
      ...
      // Auto-connect feature to auto-detect connected accounts
  ...
  // Periodic wallet status refresh to auto-sync wallet state
  useEffect(() => {
    if (!provider) return;
